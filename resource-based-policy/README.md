...
- バイパス経路をシミュレートする rogue ロール (`managed-kb-rbp-rogue-role`) を追加。identity ポリシーでは KB のデータアクションを全て Allow しており、リソースベースポリシーの Deny だけが遮断要因になる。bedrock-agentcore (rogue Gateway の実行) とアカウント (検証スクリプトの AssumeRole) の両方を信頼する
...
- メイン Gateway 側の構成 (Pre Token Generation / Interceptor / Cedar Policy 2 本) は advanced-policy と同一
...
リソースベースポリシーの仕様上の制約 (公式ドキュメント + 実測):
...
| KB リソースベースポリシー         | 正規 Gateway 実行ロール以外の Retrieve を明示 Deny | KB への全経路 (経路非依存) |
...
| #   | 経路                        | 操作                             | 期待                                     |
| --- | --------------------------- | -------------------------------- | ---------------------------------------- |
| 0   | デプロイヤ                  | GetResourcePolicy                | ポリシーがアタッチされている             |
...
全 13 チェックが PASS。記事の脚注で未検証としていた 2 点は次のとおり確定した。
